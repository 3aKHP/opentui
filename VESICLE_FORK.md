...
| #4 | `fork-npm-release.yml` placed `ZIG_LOCAL_CACHE_DIR: ${{ runner.temp }}/zig-cache` in job-level `env`; the `runner` context is unavailable there, so GitHub invalidated the workflow and every tag run after 3acf6579 failed with zero jobs (zv6 shipped via the local token fallback, without provenance). Upstream `release.yml` was also firing on every `-zv*` tag (hung at "Prepare Release", nothing published). Fixed workflow-side only at 2e78f523 — step-level env, anchored trigger glob, first-step tag gate, upstream `-zv*` exclusion — no `-zv(N+1)` release needed. Probe tag `v0.5.3-zv-ci-probe` (run 32124960266) proved job creation with the gate refusing before any build or publish step. |
